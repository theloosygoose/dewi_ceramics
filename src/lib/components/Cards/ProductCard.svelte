...
{#if typeof product != undefined}
    <a on:mouseover={hoverStateHandler} on:focus={hoverStateHandler} sveltekit:prefetch in:blur="{{delay:1+(i*200),duration:600, easing:cubicInOut}}" class="drop-shadow-sm relative overflow-x-hidden bg-slate-400 aspect-w-1 aspect-h-1 hover:scale-[1.05] transition ease-in-out " href="/store/{product.handle}">
